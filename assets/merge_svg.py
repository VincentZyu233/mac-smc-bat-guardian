...
def generate_grid_svg():
    ...
    for f in files:
        ...
        if ext == ".png":
            ...
            img_data.append(f"data:image/png;base64,{encoded}")
        else:
            with open(path, 'r', encoding='utf-8') as fh:
                encoded = base64.b64encode(fh.read().encode('utf-8')).decode('utf-8')
            img_data.append(f"data:image/svg+xml;base64,{encoded}")

    if len(img_data) == 4:
        final_svg = svg_template.format(
            img0=img_data[0], img1=img_data[1], img2=img_data[2], img3=img_data[3]
        )
